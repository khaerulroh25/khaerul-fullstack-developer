import { PrismaClient, Role, JobType, ExperienceLevel, JobStatus, ApplicationStatus } from '@prisma/client';
...
export async function seedDatabase(client: PrismaClient = prisma) {
  console.log('🌱 Memulai proses Seeding Database IndoKerja.id...');
  ...
  const userRecruiterGoto = await client.user.create({
    data: {
      email: 'recruiter.goto@indokerja.id',
  ...
  const userRecruiterTraveloka = await client.user.create({
    data: {
      email: 'recruiter.traveloka@indokerja.id',
      passwordHash: defaultPassword,
  ...
  const userRecruiterMandiri = await client.user.create({
    data: {
      email: 'recruiter.mandiri@indokerja.id',
      passwordHash: defaultPassword,
  ...
  console.log('   - Seeker:    pelamar@indokerja.id / Password123!');
  console.log('   - Recruiter: recruiter.goto@indokerja.id / Password123!');
    ...
    data: [
      {
        ...
        changedBy: 'SYSTEM',
        comment: 'Berkas lamaran berhasil diajukan ke sistem IndoKerja.id',
      ...
      },
      {
        ...
        changedBy: 'Budi Raharja',
        comment: 'Undangan Technical Interview dikirim via email untuk posisi Senior Fullstack Engineer.',
  ...
  const app2 = await client.application.create({
    data: {
      ...
      portfolioUrl: 'https://khaerul.dev',
      resumeUrl: 'https://storage.indokerja.id/resumes/khaerul-anam-cv.pdf',
      coverLetter: 'Tertarik mendalami pengembangan design system dan web performance di Traveloka.',
      expectedSalary: 18000000,
      noticePeriod: 'Immediately',
      status: ApplicationStatus.SCREENING,
      recruiterNotes: 'Pengalaman frontend sangat cocok dengan kebutuhan tim UI Platform.',
    ...
    data: [
      {
        ...
        newStatus: ApplicationStatus.SUBMITTED,
        changedBy: 'SYSTEM',
        comment: 'Berkas lamaran berhasil masuk ke sistem IndoKerja.id',
        timestamp: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000),
      },
      {
        applicationId: app2.id,
        ...
        comment: 'Sedang meninjau kesesuaian pengalaman Storybook & Accessibility.',
  ...
  const app3 = await client.application.create({
    data: {
      ...
      resumeUrl: 'https://storage.indokerja.id/resumes/ahmad-rizki-cv.pdf',
      coverLetter: 'Melamar posisi Senior Fullstack Engineer dengan fokus Node.js & React.',
  ...
  console.log('📄 Berkas lamaran dan audit log status berhasil dibuat.');
