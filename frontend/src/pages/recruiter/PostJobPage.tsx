...
interface PostJobPageProps {
  ...
  onSubmitJob: (jobData: Omit<Job, "id" | "createdAt">) => Promise<boolean> | boolean | void;
}

export const PostJobPage: React.FC<PostJobPageProps> = React.memo(
  ({ currentUser, onNavigateBack, onSubmitJob }) => {
      ...
      const fetchCompanies = async () => {
        try {
          ...
          if (liveCompanies && liveCompanies.length > 0) {
            ...
            // Pilih perusahaan yang dimiliki oleh akun perekrut ini, atau default pertama
            const owned = liveCompanies.find((c) => currentUser && c.userId === currentUser.id);
            setSelectedCompanyId(owned ? owned.id : liveCompanies[0].id);
          }
        } catch (err) {
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
            ...
            <p className="mt-1.5 text-xs sm:text-sm text-slate-500">
              Lengkapi rincian berikut untuk mempublikasikan lowongan kerja ke
              ribuan talenta di{" "}
              <strong className="text-slate-700">yukKerja</strong>.
            </p>
          </div>

          {/* Banner Peringatan Jika Bukan Akun Recruiter */}
          {(!currentUser || currentUser.role !== "RECRUITER") && (
            <div className="mb-6 flex items-center gap-3.5 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-xs sm:text-sm text-amber-900 shadow-sm">
              <AlertTriangle className="h-5 w-5 text-amber-600 shrink-0" />
              <div>
                <strong>Akses Khusus Perekrut (HR):</strong> Pastikan Anda telah masuk dengan akun berstatus <strong>Perekrut (Recruiter)</strong> agar lowongan dapat berhasil dipublikasikan ke server.
              </div>
            </div>
          )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full cursor-pointer rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-xs sm:text-sm font-medium text-slate-800 outline-none transition focus:border-amber-400 focus:bg-white focus:ring-2 focus:ring-amber-100"
                    >
                      {JOB_TYPE_OPTIONS.filter((o) => o.value !== "").map(
                        (opt) => (
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full cursor-pointer rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-xs sm:text-sm font-medium text-slate-800 outline-none transition focus:border-amber-400 focus:bg-white focus:ring-2 focus:ring-amber-100"
                    >
                      {EXPERIENCE_LEVEL_OPTIONS.filter(
                        (o) => o.value !== "",
                      ).map((opt) => (
                        <option key={opt.value} value={opt.value}>
                          {opt.label}
                ...
                <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-4">
                  <div>
                    ...
                    <div className="mt-0.5 text-[11px] text-slate-500">
                      Lowongan dengan transparansi gaji memperoleh 2.5x lebih
                      banyak pelamar berkualitas.
                    </div>
                  </div>
                  <input
                    type="checkbox"
                ...
                {isSalaryDisclosed && (
                  <div>
                    ...
                    <span className="mt-2 block text-[11px] text-slate-500 font-medium">
                      💡 Pratinjau Tampilan:{" "}
                      {formatSalary(salaryMin, salaryMax, true, "/ bulan")}
                    </span>
                  </div>
                )}
              </div>
